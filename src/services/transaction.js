                                    ...
                                    () => {
                                        ...
                                        db.transaction(tx => {
                                            tx.executeSql(
                                                ...
                                                [pin, email],
                                                () => { resolve() },
                                                (_, err) => { console.log(err); reject(err) }
                                            );
                                        })
                                    },
                                    (_, err) => { console.log(err); reject(err) }
    ...
    return new Promise((resolve, reject) => {
        ...
        db.transaction(tx => {
            tx.executeSql(
                ...
                (_, err) => { console.log(err); reject("error fetching") }
            );
        });
    });
}
...
const insertInward = async (receivedDate, paymentDate, location, lotnumber, item, quanity, unit, marka, balance, imageExist, imageName) => {
    return new Promise((resolve, reject) => {
        ...
        db.transaction(tx => {
            tx.executeSql(
                ...
                [receivedDate, paymentDate, location, lotnumber, item, quanity, unit, marka, balance, imageExist, imageName],
                () => { resolve() },
...
const insertOutward = async (receivedDate, gatePassDate, location, lotnumber, item, quantity, issued, unit, marka, balance, receivedId, gatepass) => {
    return new Promise((resolve, reject) => {
        db.transaction(tx => {
            tx.executeSql(
                ...
                [receivedDate, gatePassDate, location, lotnumber, item, quantity, issued, unit, marka, balance, receivedId, gatepass],
                () => {
                    tx.executeSql(
...
const updateInward = async (receivedDate, paymentDate, location, lotnumber, item, quanity, unit, marka, balance, id, imageExist, imageName) => {
    return new Promise((resolve, reject) => {
        ...
        db.transaction(tx => {
            tx.executeSql(
                ...
                [receivedDate, paymentDate, location, lotnumber, item, quanity, unit, marka, balance, imageExist, imageName, id],
                () => { resolve() },
                (_, err) => { console.log(err); reject(err) }
            );
        })
...
const updateOutwards = async (receivedDate, gatePassDate, location, lotnumber, item, quantity, issued, unit, marka, balance, id, receivedId) => {
    return new Promise((resolve, reject) => {
        ...
        db.transaction(tx => {
            tx.executeSql(
                ...
                [receivedDate, gatePassDate, location, lotnumber, item, quantity, issued, unit, marka, balance, id],
                () => {
                    tx.executeSql(
                        "update Inward set balance=? where id=?",
                        [balance, receivedId],
...
const getInwardReport = async (fromDate, toDate, groupBy, orderby) => {
    var query = "select * from Inward where receivedDate >= ? and receivedDate <= ? ORDER by ?";

    if (groupBy) {
        query = "select * from Inward where receivedDate >= ? and receivedDate <= ? ORDER by item,?";
    }
...
const getOutwardReport = async (fromDate, toDate, groupBy, orderby) => {
    var query = "select * from Outward where gatePassDate >= ? and gatePassDate <= ? ORDER by ?";

    if (groupBy) {
        query = "select * from Outward where gatePassDate >= ? and gatePassDate <= ? ORDER by item,?";
    }
    ...
    return new Promise((resolve, reject) => {
        db.transaction(tx => {
            tx.executeSql(
                ...
                [],
                (_, { rows: { _array } }) => { resolve(_array) },
                (_, err) => { reject("error fetching"); console.log(err) }
            );
        })
    });
}
...
const getStockPosition = async (reportType, findLotNumber, order, toDate, item, selectedItems) => {

    var query = "select a.id, a.receivedDate, a.paymentDate,a.location,a.lotnumber,a.item,a.quantity,a.unit,a.marka,a.balance, b.gatePassDate,b.issued,b.gatepass from Inward as a LEFT JOIN Outward as b on a.id = b.receivedId where ((b.gatePassDate is null and a.receivedDate<=" + toDate + ") or (b.gatePassDate is not null and b.gatePassDate <=" + toDate + "))";
                ...
                Object.keys(data).forEach((tableName) => {
                    if (tableName !== 'User') {
                        data[tableName].forEach((row) => {
                            ...
                            tx.executeSql(`INSERT INTO ${tableName} (${columns}) VALUES (${placeholders});`, values);
                        });
                    }
                });
            },
