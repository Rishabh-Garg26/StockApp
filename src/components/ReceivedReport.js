...
const ReceivedReport = () => {
    ...
    const getReceivedReportPdf = async () => {
        ...
        try {
            const res = await Transaction.getInwardReport(moment.utc(fromDate).valueOf(), moment.utc(toDate).valueOf(), groupBy, orderby);
            // console.log(res);
            setLoader(true);
            if (res.length === 0) {
                Alert.alert(
        ...
        <ScrollView>
            <View style={styles.container}>
                ...
                <Dropdown
                    ...
                    data={[{ label: 'As Entered', value: 'id' }, { label: 'Lotwise', value: 'lotnumber' }, { label: 'DateWise', value: 'receivedDate' }]}
                    search
                    maxHeight={300}
                    labelField="label"
                    valueField="value"
